    ...
    while (pawns) {
        ...
        if (!captures_only) {
            ...
            if (!get_bit(all_pieces, to)) {
                if (get_rank(to) == promotion_rank) {
                    moves.emplace_back(create_move(from, to, pos.piece_on_square(from), NO_PIECE, Move::QUEEN_PROMOTION));
                    moves.emplace_back(create_move(from, to, pos.piece_on_square(from), NO_PIECE, Move::ROOK_PROMOTION));
                    moves.emplace_back(create_move(from, to, pos.piece_on_square(from), NO_PIECE, Move::BISHOP_PROMOTION));
                    moves.emplace_back(create_move(from, to, pos.piece_on_square(from), NO_PIECE, Move::KNIGHT_PROMOTION));
                } else {
                    moves.emplace_back(create_move(from, to, pos.piece_on_square(from)));
                }
                ...
                if (r == dbl_push_rank) {
                    ...
                    if (!get_bit(all_pieces, to)) {
                        moves.emplace_back(create_move(from, to, pos.piece_on_square(from), NO_PIECE, Move::NO_PROMOTION, Move::DOUBLE_PAWN_PUSH_FLAG));
                    }
                }
            }
        }
        ...
        while (attacks) {
            ...
            if (to == pos.en_passant_sq) {
                moves.emplace_back(create_move(from, to, pos.piece_on_square(from), (us == WHITE ? BP : WP), Move::NO_PROMOTION, Move::EN_PASSANT_FLAG));
            } else {
                PieceType captured = pos.piece_on_square(to);
                if (get_rank(to) == promotion_rank) {
                     moves.emplace_back(create_move(from, to, pos.piece_on_square(from), captured, Move::QUEEN_PROMOTION));
                     moves.emplace_back(create_move(from, to, pos.piece_on_square(from), captured, Move::ROOK_PROMOTION));
                     moves.emplace_back(create_move(from, to, pos.piece_on_square(from), captured, Move::BISHOP_PROMOTION));
                     moves.emplace_back(create_move(from, to, pos.piece_on_square(from), captured, Move::KNIGHT_PROMOTION));
                } else {
                    moves.emplace_back(create_move(from, to, pos.piece_on_square(from), captured));
                }
            }
        }
    }
...
void generate_piece_moves(const Position& pos, MoveList& moves, bool captures_only) {
    ...
    while (our_pieces) {
        ...
        Bitboard targets = captures_only ? (attacks & their_pieces) : attacks;

        while (targets) {
            Square to = pop_bit(targets);
            PieceType captured = get_bit(their_pieces, to) ? pos.piece_on_square(to) : NO_PIECE;
            moves.emplace_back(create_move(from, to, Pt, captured));
        }
    }
}
...
void generate_castling_moves(const Position& pos, MoveList& moves) {
    ...
    if (us == WHITE) {
        if ((pos.castling_rights & WHITE_KINGSIDE) &&
            ...
            moves.emplace_back(create_move(E1, G1, WK, NO_PIECE, Move::NO_PROMOTION, Move::CASTLING_FLAG));
        }
        if ((pos.castling_rights & WHITE_QUEENSIDE) &&
            ...
            moves.emplace_back(create_move(E1, C1, WK, NO_PIECE, Move::NO_PROMOTION, Move::CASTLING_FLAG));
        }
    } else { // BLACK
        if ((pos.castling_rights & BLACK_KINGSIDE) &&
            ...
            moves.emplace_back(create_move(E8, G8, BK, NO_PIECE, Move::NO_PROMOTION, Move::CASTLING_FLAG));
        }
        if ((pos.castling_rights & BLACK_QUEENSIDE) &&
            ...
            moves.emplace_back(create_move(E8, C8, BK, NO_PIECE, Move::NO_PROMOTION, Move::CASTLING_FLAG));
        }
    }
}
